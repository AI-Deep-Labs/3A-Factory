...
Tài liệu vận hành cho phiên bản **3.1.0-rc.2**. Spec = package dưới `docs/tasks/`, không phải single document.
...
| 11 | Spec approval | User xác nhận (có/không) | `approved` |
...
| # | Phase | Skill / action | Kết quả chính |
|---|---|---|---|
| 12 | Task selection | `project-manager` (auto hoặc `/project-manager`) | Chọn task ready (dependency OK) |
...
| 19 | User review | User xác nhận nghiệm thu (có/không) | `done` |
| 20 | Done | — | Feature closed về workflow |
| 21 | Deploy | `/deploy` + xác nhận deploy | Deploy riêng; không gộp với bước 19 |
...
| `awaiting_approval` | `approved` | user xác nhận spec package |
...
| `awaiting_user_review` | `done` | user xác nhận nghiệm thu |
| `*` | `blocked` / `rejected` / `cancelled` | policy / reject / cancel |

## Approval gate table

Agent hỏi **câu xác nhận** tại mỗi gate; user trả lời có/không hoặc ngôn ngữ tự nhiên. Token `APPROVED_*` là ID nội bộ (optional). Contract § 5.4.1.

| Gate | Token (internal) | Precondition | Effect |
|---|---|---|---|
